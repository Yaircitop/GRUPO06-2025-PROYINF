...
class TestLoginEndpoint(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.base_url = "http://127.0.0.1:8000/signup/register/"
    ...
    def test_registro_valido(self):
        ...
        print(response)
        print("Registro Realizado Exitosamente - Código esperado 302 - Código Obtenido:", response.status_code)
    ...
    def test_registro_invalido(self):
        ...
        print(response)
        print("Registro inválido - Código esperado 200 - Código Obtenido:", response.status_code)
        self.assertEqual(response.status_code, 200)
            
        self.assertIn("El RUN ya está registrado", response.text)

    @classmethod
    def tearDownClass(cls):
        # Limpiar datos insertados durante las pruebas
        cls.user_collection.delete_many({"username": {"$regex": "test"}})
        cls.profile_collection.delete_many({"run": {"$regex": ".*"}})
